            ...
            <form action="<?= base_url() ?>rent_info/delete" method="POST">
                <div class="box">
                    <div class="box-header">
                        <div class="col-md-12">
                            <a href="<?= base_url('rent_info/add') ?>" class="btn btn-success btn-flat" name="btn_submit" value="เพิ่มข้อมูล">เพิ่มข้อมูล</a>
                                    ...
                                    <?php if (isset($resultq1)) { ?>
                                        <?php foreach ($resultq1 as $rows) { ?>
                                            <tr class="<?= $rows['status'] == 'เปิดสัญญา' ? 'bg-success' : 'bg-danger' ?>">
                                                ...
                                                <td>
                                                    <a href="<?= base_url() ?>rent_info/edit/<?= $rows['id'] ?>" class="btn btn-sm btn-primary">ดูรายละเอียดการเช่า</a>

                                                    <a href="#" onclick="if (confirm('คุณต้องการลบข้อมูลนี้หรือใม่?') == true) {
                                                                        document.location.href = '<?= base_url() ?>rent_info/delete/<?= $rows['id'] ?>'
                                                                                }" class="btn btn-sm btn-danger hidden">ลบ</a>
